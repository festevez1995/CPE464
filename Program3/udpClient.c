...
int main (int argc, char *argv[])
 {
  ...
  clientSetup(argc, argv, client);

	// portNumber = checkArgs(argc, argv);
  // client->remotePort = portNumber;

	// socketNum = setupUdpClientToServer(&server, client->remoteMachine, portNumber);
  /* A call to sendtoErr_init function*/
  sendErr_init(client->errorPercent, DROP_ON, FLIP_ON, DEBUG_ON, RSEED_OFF);
  //
	// talkToServer(socketNum, &server);
  //
	// close(socketNum);
  startProcessingRequests(client);
